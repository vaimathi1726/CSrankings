...
class CSRankings {
    ...
    analyzeTop40MarginalDelta(topN = 5) {
        ...
        try {
            ...
            this.createMarginalDeltaChart(areaCounts, "Top 1 Faculty per School (Top 40) by Marginal Delta", "marginal-delta-chart-1", this.marginalCalculationMethod);
            this.createMarginalDeltaChart(top5AreaCounts, `Top ${topN} Faculty per School (Top 40) by Marginal Delta`, "marginal-delta-chart-5", this.marginalCalculationMethod);
    ...
    analyzeAverageMarginalDeltaByArea() {
        ...
        try {
            ...
            for (const dept in this.currentDeptNames) {
                ...
                for (const name of faculty) {
                    ...
                    if (marginal > 0) {
                        ...
                        const marginalDeltaPercent = deptScore > 0 ? (marginal / deptScore) * 100 : 0;
                        ...
                        if (primaryArea && primaryArea !== "Unknown") {
                            ...
                            areaMarginalData[primaryArea].marginals.push(marginalDeltaPercent);
                        }
                    }
                }
            }
            // Calculate averages
            const averageMarginals = {};
            ...
            this.createAverageMarginalDeltaChart(averageMarginals, this.marginalCalculationMethod);
                ...
                for (const year in areaYearData[area]) {
                    ...
                    for (const name in facultyPrimaryArea) {
                        if (facultyPrimaryArea[name] === area) {
                            const pubs = ((_a = facultyYearPubs[name]) === null || _a === void 0 ? void 0 : _a[yearNum]) || 0;
                            if (pubs > 0) {
                                facultyWithPubs++;
                                totalPubs += pubs;
                            }
    ...
    generateProgressiveShades(baseColor, count, darkToLight = true) {
        ...
        for (let i = 0; i < count; i++) {
            ...
            shades.push(`#${newR.toString(16).padStart(2, "0")}${newG
                .toString(16)
    ...
    createAverageMarginalDeltaChart(averageMarginals, method) {
        ...
        console.log(`Creating average marginal delta chart with data:`, averageMarginals);
            ...
            if (successDiv && successDiv.parentNode) {
                ...
                if (buttonsDiv) {
                    ...
                    if (buttonsDiv.nextSibling) {
                        successDiv.parentNode.insertBefore(tableContainer, buttonsDiv.nextSibling);
                    }
                    else {
