...
class CSRankings {
  ...
  public static readonly regions: Array<string> = [
    "europe",
    "northamerica",
    "southamerica",
    "australasia",
  ...
  private readonly allowRankingChange =
    false; /* Can we change the kind of rankings being used? */
  ...
  private readonly interdisciplinaryAreas = [
    "bio",
    "graph",
    "csed",
    "ecom",
  ...
  private readonly BarChartIcon =
    "<img class='closed_chart_icon chart_icon' alt='closed chart' src='png/barchart.png'>"; // bar chart image
  private readonly OpenBarChartIcon =
    "<img class='open_chart_icon chart_icon' alt='opened chart' src='png/barchart-open.png'>"; // opened bar chart image
  private readonly PieChartIcon =
    "<img class='closed_chart_icon chart_icon' alt='closed chart' src='png/piechart.png'>";
  private readonly OpenPieChartIcon =
    "<img class='open_chart_icon chart_icon' alt='opened chart' src='png/piechart-open.png'>";
  ...
  private async loadACMFellow(acmfellow: {
    [key: string]: number;
  }): Promise<void> {
    const data = await new Promise((resolve) => {
  ...
  private async loadCountryNames(countryNames: {
    [key: string]: string;
  }): Promise<void> {
    const data = await new Promise((resolve) => {
  ...
  private inRegion(dept: string, regions: string): boolean {
    switch (regions) {
      case "us":
  ...
  private activateFields(value: boolean, fields: Array<number>): boolean {
  ...
  private facultySortPreference: {
    [dept: string]: "marginal" | "adjusted" | "pubs" | "marginalpct" | "adjpct";
  } = {};
  ...
  private buildDropDown(
    ...
    facultycount: { [key: string]: number },
    facultyAdjustedCount: { [key: string]: number },
    facultyMarginal?: { [key: string]: number }
  ): { [key: string]: string } {
    ...
    for (const dept in deptNames) {
      ...
      let p =
        ...
        `style="color: ${
          currentSort === "pubs" ? "#0066cc" : "#4a5568"
        }; font-weight: ${
          currentSort === "pubs" ? "bold" : "normal"
        }; text-decoration: underline; cursor: pointer;" ` +
        `title="Click to sort by publication count">#&nbsp;Pubs${
          currentSort === "pubs" ? " ▼" : ""
        }</a></em></small>` +
        "</th>" +
        '<th align="right">' +
        ...
        `style="color: ${
          currentSort === "adjusted" ? "#0066cc" : "#4a5568"
        }; font-weight: ${
          currentSort === "adjusted" ? "bold" : "normal"
        }; text-decoration: underline; cursor: pointer;" ` +
        `title="Click to sort by adjusted count">Adj.&nbsp;#${
          currentSort === "adjusted" ? " ▼" : ""
        }</a></em></small>` +
        "</th>" +
        '<th align="right">' +
        `<small><em><a href="#" onclick="csr.sortFaculty('${dept}', 'marginal'); return false;" ` +
        `style="color: ${
          currentSort === "marginal" ? "#0066cc" : "#4a5568"
        }; font-weight: ${
          currentSort === "marginal" ? "bold" : "normal"
        }; text-decoration: underline; cursor: pointer;" ` +
        `title="Click to sort by marginal contribution">Marginal&nbsp;Δ${
          currentSort === "marginal" ? " ▼" : ""
        }</a></em></small>` +
        "</th>" +
        '<th align="right">' +
        `<small><em><a href="#" onclick="csr.sortFaculty('${dept}', 'marginalpct'); return false;" ` +
        `style="color: ${
          currentSort === "marginalpct" ? "#0066cc" : "#4a5568"
        }; font-weight: ${
          currentSort === "marginalpct" ? "bold" : "normal"
        }; text-decoration: underline; cursor: pointer;" ` +
        `title="Click to sort by marginal percentage">Marginal&nbsp;%${
          currentSort === "marginalpct" ? " ▼" : ""
        }</a></em></small>` +
        "</th>" +
        '<th align="right">' +
        `<small><em><a href="#" onclick="csr.sortFaculty('${dept}', 'adjpct'); return false;" ` +
        `style="color: ${
          currentSort === "adjpct" ? "#0066cc" : "#4a5568"
        }; font-weight: ${
          currentSort === "adjpct" ? "bold" : "normal"
        }; text-decoration: underline; cursor: pointer;" ` +
        `title="Click to sort by adjusted percentage">Adj&nbsp;%${
          currentSort === "adjpct" ? " ▼" : ""
        }</a></em></small>` +
        "</th>" +
        "</tr>" +
      ...
      const marginals = keys.map((name) =>
        facultyMarginal && name in facultyMarginal ? facultyMarginal[name] : 0
      );
      ...
      const deptAdjTotal = deptNames[dept].reduce(
        (sum, name) => sum + (facultyAdjustedCount[name] || 0),
        0
      );
      ...
      const sortedEntries = keys
        .map((name) => {
          ...
          const myMarginal = facultyMarginal?.[name] ?? 0;
      ...
      for (const name of keys) {
        ...
        p += `<span class="areaname">${this.areaString(
          name
        ).toLowerCase()}</span>&nbsp;`;
        ...
        p += `<span onclick='csr.toggleChart("${escape(
          name
        )}"); ga("send", "event", "chart", "toggle", "toggle ${escape(
          name
        )} ${$("#charttype")
          .find(":selected")
          .val()} chart");' title="Click for author's publication profile." class="hovertip" id="${
          escape(name) + "-chartwidget"
        }">`;
        p +=
          this.ChartIcon +
          "</span>" +
        ...
        if (facultyMarginal && name in facultyMarginal) {
          ...
          marginalTooltip = `Absolute drop: ${absMarginal} (from dept score ${deptScore.toFixed(
            2
          )})`;
        }
  ...
  private buildOutputString(
    ...
    if (numAreas > 0) {
      ...
      for (let ind = 0; ind < keys2.length; ind++) {
        ...
        const country =
          this.countryNames[abbrv.toUpperCase()] ?? abbrv.toUpperCase();

        s +=
          "&nbsp;" +
          `<span onclick="csr.toggleFaculty('${esc}');">${dept}</span>` +
          `&nbsp;<img  title="${country}" src="/flags/${abbrv}.png">&nbsp;` +
          `<span class="hovertip" onclick='csr.toggleChart("${esc}"); ga("send", "event", "chart", "toggle-department", "toggle ${esc} ${$(
            "#charttype"
          )
            .find(":selected")
        ...
        s += `<td align="right">${(Math.round(10.0 * v) / 10.0).toFixed(
          1
        )}</td>`;
        ...
        const deptFacultyMarginals = deptNames[dept]
          .map((name) =>
            facultyMarginal && name in facultyMarginal
  ...
  public rank(update: boolean = true): boolean {
    ...
    let deptNames: { [key: string]: Array<string> } =
      {}; /* names of departments. */
    let deptCounts: { [key: string]: number } =
      {}; /* number of faculty in each department. */
    let facultycount: { [key: string]: number } =
      {}; /* name -> raw count of pubs per name / department */
    let facultyAdjustedCount: { [key: string]: number } =
      {}; /* name -> adjusted count of pubs per name / department */
    let currentWeights: { [key: string]: number } =
      {}; /* array to hold 1 or 0, depending on if the area is checked or not. */
    ...
    if (numAreas > 0) {
      for (const dept in deptNames) {
        ...
        for (const name of deptNames[dept]) {
          ...
          for (const area in CSRankings.topLevelAreas) {
            if (currentWeights[area] === 0) continue;
            const areaDept = area + dept;
            const total =
              areaDept in this.areaDeptAdjustedCount
                ? this.areaDeptAdjustedCount[areaDept]
                : 0;
            const contrib =
              name in this.facultyAreaAdjustedCount &&
            ...
            if (contrib > 0) {
              ...
              for (const a in CSRankings.topLevelAreas) {
                if (currentWeights[a] === 0) continue;
                const aDept = a + dept;
                const aTotal =
                  aDept in this.areaDeptAdjustedCount
          ...
          if (areaMarginals.length > 0) {
            switch (this.marginalCalculationMethod) {
              ...
              case "original":
                ...
                const newstat = Math.pow(prod, 1 / numAreas);
                this.facultyMarginal[name] = Math.max(
                  0,
                  this.stats[dept] - newstat
                );
                break;
            }
          } else {
    ...
    const univtext = this.buildDropDown(
      deptNames,
      facultycount,
      facultyAdjustedCount,
      this.facultyMarginal
    ...
    const s = this.buildOutputString(
      numAreas,
      this.countryAbbrv,
      deptCounts,
      deptNames, // ADD this parameter
      univtext,
  ...
  public sortFaculty(
    dept: string,
    sortBy: "marginal" | "adjusted" | "pubs" | "marginalpct" | "adjpct"
  ): void {
  ...
  private getPrimaryResearchArea(name: string): string {
    if (!this.authorAreas[name]) {
      return "Unknown";
    }
  ...
  private getPrimaryResearchAreaKey(name: string): string | null {
    if (!this.authorAreas[name]) {
      return null;
    }
    // Find the area with maximum contribution
    let maxArea: string | null = null;
    let maxCount = 0;
  ...
  public setMarginalMethod(method: "weighted" | "primary" | "original"): void {
  ...
  public analyzeTop40MarginalDelta(topN: number = 5): void {
    ...
    try {
      ...
      const top5AreaCounts: { [area: string]: number } = {};

      for (const school of top40Schools) {
        if (!(school in deptNames)) continue;
  ...
  public analyzeAverageMarginalDeltaByArea(): void {
    ...
    try {
      ...
      const chartMethodSelect = document.getElementById(
        "average-marginal-delta-chart-method"
      ) as HTMLSelectElement;
      if (chartMethodSelect) {
        this.marginalCalculationMethod = chartMethodSelect.value as
          | "weighted"
          | "primary"
          | "original";
      } else {
  ...
  public analyzeAveragePubsByAreaOverTime(): void {
    ...
    try {
      ...
      const facultyPrimaryArea: { [name: string]: string } = {};
      ...
      const areaYearData: {
        [area: string]: {
          [year: number]: { totalPubs: number; facultyCount: number };
        };
      } = {};
      ...
      // (needed for accurate averaging - only count faculty who actually published)
      const facultyYearPubs: {
        [name: string]: { [year: number]: number };
      } = {};
        ...
        for (const year in areaYearData[area]) {
          ...
          for (const name in facultyPrimaryArea) {
            if (facultyPrimaryArea[name] === area) {
              const pubs = facultyYearPubs[name]?.[yearNum] || 0;
              if (pubs > 0) {
                facultyWithPubs++;
                totalPubs += pubs;
              }
      ...
      for (const area in areaYearData) {
        for (let year = startyear; year <= endyear; year++) {
          if (year in areaYearData[area]) {
            ...
            const average =
              data.facultyCount > 0 ? data.totalPubs / data.facultyCount : 0;
            chartData.push({
              area: area,
              year: year,
  ...
  private generateProgressiveShades(
    baseColor: string,
    count: number,
  ...
  private createAveragePubsOverTimeChart(
    chartData: Array<{ area: string; year: number; average: number }>,
    startyear: number,
    endyear: number
  ): void {
    ...
    const systemsShades = this.generateProgressiveShades(
      "#cc6600",
      systemsAreaOrder.length
    ); // Dark orange base
    const theoryShades = this.generateProgressiveShades(
      "#2d5a27",
      theoryAreaOrder.length
    ); // Dark green base
    const interdisciplinaryShades = this.generateProgressiveShades(
      "#6a1b9a",
      interdisciplinaryAreaOrder.length
    ); // Dark purple base
  ...
  private createAverageMarginalDeltaChart(
    ...
    if (!chartDivElement) {
      console.error(`Chart div ${chartDivId} not found`);
      return;
    }
  ...
  private createMarginalDeltaChart(
    ...
    if (!method) {
      const methodSelect = document.getElementById("marginal-method") as HTMLSelectElement;
      method = methodSelect ? (methodSelect.value as "weighted" | "primary" | "original") : this.marginalCalculationMethod;
    }
          ...
          tableHTML += `
            <tr style="background-color: ${
              ...
              <td style="padding: 6px; border: 1px solid #ddd; ${
                !schoolNameShown ? "font-weight: bold;" : ""
              }">${!schoolNameShown ? i + 1 : ""}</td>
              <td style="padding: 6px; border: 1px solid #ddd; ${
                !schoolNameShown ? "font-weight: bold;" : ""
              }">${!schoolNameShown ? school : ""}</td>
              <td style="padding: 6px; border: 1px solid #ddd;">
                ${f.name} ${
            isTop1
              ...
              <td style="padding: 6px; border: 1px solid #ddd; text-align: right; font-family: monospace; font-weight: bold;">${f.marginal.toFixed(
                2
              )}</td>
  ...
  public toggleMarginalMethod(): void {
    // Cycle through methods: weighted -> primary -> original -> weighted
    const methods: Array<"weighted" | "primary" | "original"> = [
      "weighted",
      "primary",
      "original",
    ];
  ...
  private showMethodNotification(): void {
    const methodNames = {
    ...
    notification.innerHTML = `Using: ${
      methodNames[this.marginalCalculationMethod]
    }`;
  ...
  public static geoCheck(): void {
    navigator.geolocation?.getCurrentPosition((position) => {
      const continent = whichContinent(
  ...
  public static subsetting(sibs: [string]): boolean {
    ...
    const subsettedAbove =
      numCheckedAbove > 0 && numCheckedAbove < aboveFold.length;
